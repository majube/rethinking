...
precis(m6.4)
plot(precis(m6.4))
post_m6.3 <- extract.samples(m6.3)
post_m6.4 <- extract.samples(m6.4)
plot(post_m6.3$bF ~ post_m6.4$bL, col=col.alpha(rangi2, 0.1), pch=16)
cor(post_m6.3$bF, post_m6.4$bL)
#m6.10
m6.5 <- quap(
  alist(
    K ~ dnorm(mu, sigma),
...
coordinates(dag_6.1) <- list(x=c(U=0, X=0, A=0.5, B=0.5, C=1, Y=1), y=c(X=0, Y=0, U=-1, C=-1, A=-1.25, B=-0.75))
drawdag(dag_6.1)
...
coordinates(dag_6.2) <- list(x=c(A=0, S=0, M=0.5, W=1, D=1), y=c(A=0, D=0, M=-0.5, S=-1, W=-1))
drawdag(dag_6.2)
