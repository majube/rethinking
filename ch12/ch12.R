...
plot(dat2$P, dat2$T, xlab="population", ylab="total tools",
     ...
     xlim=c(0, max(dat2$P+1e4)), ylim=c(0, 80), cex=1+normalize(k))

ns <- 100
P_seq <- seq(from=min(dat2$P), to=max(dat2$P)+1e4, length.out=ns)

# predictions for cid=1 (low contact)
lambda <- link(m12.2, data=data.frame(P=P_seq, cid=1))
...
# order seems wrong - should be:
edu_levels2 <- c(2, 6, 8, 4, 7, 1, 5, 3)
d$edu_new2 <- edu_levels[d$edu]
levels(d$edu)[edu_levels2] # correct
...
dat <- list(
  ...
  contact = d$contact,
  E = as.integer(d$edu_new2),   # edu_new2 as an index
...
#12.37
dat$edu_norm <- normalize(d$edu_new2) # use d$edu_new2 again
m12.7 <- ulam(
  alist(
