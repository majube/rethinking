...
There are two paths from *area* to *weight*: $area \to avgfood \to groupsize \to weight$ and $area \to avgfood \to weight$. So I'll use the following model:
...
for (i in 1:N) {
  curve(a[i] + bA[i]*x, from=-3, to=3, add=TRUE, col=col.alpha("black", 0.2))
}
```
